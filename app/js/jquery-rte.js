...
$.fn.richTextEditor = function() {
	...
	this.each(function() {
		...
		var editorCode = $('<div/>', { class: 'rich_text'});

		editorCode
			.append($('<input/>', {type: 'hidden', name: textarea.name, value: textarea.value}))
			.append($('<ul/>', {class: 'rich_text_toolbar'}))
			.append($('<div/>', {class: 'rich_text_editor', contenteditable: true}));

		$(this).replaceWith(editorCode);

		var $input		= editorCode.children('input'),
			$toolbar	= editorCode.children('.rich_text_toolbar'),
			$editor		= editorCode.children('.rich_text_editor');
		...
		for(key in toolbarButtons) {
			...
			if (toolbarButtons[key].list && typeof(toolbarButtons[key].list) === 'object') {
				appendStr += '<li><button data-exec="toggle" class="sublist">' + toolbarButtons[key].label + '</button><ul class="hidden">';
