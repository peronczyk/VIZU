...
class Language {
	...
	public function get_list() {
		if (!$this->lang_list) {
			...
			if (!$result) {
				Core::displayError('Language database table does not exist. Probably application was not installed properly. Please run <a href="install/">installation</a> process.', __FILE__, __LINE__, debug_backtrace());
			}
	...
	public function set() {
		...
		if ($this->exists(\Config::$DEFAULT_LANG)) {
			...
			Core::displayError('Configured default page language does not exist in database or it is inactive.', __FILE__, __LINE__, debug_backtrace());
		}
	}
	...
	public function loadThemeTranslations() {
		...
		if (file_exists($lang_file)) {
			...
			Core::displayError('Theme translations file not found at location: ' . $lang_file, __FILE__, __LINE__);
		}
	}
